...
def permutation_p_vals(genome, reference, feature, minimum_overlap, num_permutations, prior_prob_null, prior_prob_alt):
    ...
    print(f'{ np.sum( permuted_overlap_ratios >= observed_overlap  ) } permutations resulted in the same or greater proportion of overlaps.')
...
def calculate_overlaps(genome, reference, genomic_features_bedtools, minimum_overlap):
    ...
    for key, feature in genomic_features_bedtools.items():
        print(f'Processing {key}...')
        overlaps[key] = (permutation_p_vals(genome, reference, feature, minimum_overlap, 5, 0.5, 0.5))
